...
generate_quantile <- function(n,d,x=NULL)
{
  if(is.null(x))
  {
    x=matrix(runif(n*d,0,1),n,d)
  }
  beta <- c(c(1,1,1,1,1),rep(0,d-5))
  # response
  x_n <- x%*%beta
  y=c(2*sin(pi*x_n)+pi*x_n+rnorm(nrow(x))*sqrt(1+x_n^2))
  return(list(x=x,y=y))
}
...
generate_data <- function(n,x=NULL) {generate_quantile(n=n,d=d,x=x)}

data_test_aux <- generate_data(n=n_test) # used to fix x test
plot(rowSums(data_test_aux$x[,1:5]),data_test_aux$y)
